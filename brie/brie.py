...
class Brie:
    def __init__(
        ...
        # # alongshore resolution of wave energy
        # self._wave_climl = int(180.0 / wave_angle_resolution)

        # ashton wave distribution
        self._wave_dist = ashton(
            a=self._wave_asym, h=self._wave_high, loc=-np.pi / 2, scale=np.pi
        )
        ...
        # self._y = np.arange(
        #     self._dy, self._dy * self._ny, self._dy
    ...
    # placeholder for wave_dist.setter
    ...
    def update(self):
        ...
        if self._barrier_model_on:
            ...
            pass  # do nothing, x_t_dt, x_s_dt, x_b_dt, and h_b_dt all come from Barrier3d
        ...
        if self._inlet_model_on:
            ...
            # if a time series of wave angle is provided, use that in the inlet model for each time step
            if self._bseed:
                self._inlets.wave_angle = self._wave_angle[self._time_index - 1]

            # update inlet model parameters prior to advancing by one time step
            self._inlets.shoreline_x = self._x_s
            self._inlets.bay_shoreline_x = self._x_b
            self._inlets.h_b = self._h_b
            self._inlets.basin_width = self._basin_width

            self._inlets.update()  # advance one time step
            ...
            for j in np.arange(1, np.size(self._inlet_idx) + 1):
                ...
                Qs_in[j - 1] = self._inlets.q_s[self._inlet_idx[j - 1]]
