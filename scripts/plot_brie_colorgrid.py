...
- plots output from brie.py and brie.m for comparison
...
# python outputs
output = np.load('test_brie_matlab_seed.npz', allow_pickle=True)
...
inputs_p1 = range(np.size(dt))
inputs_p2 = range(np.size(dy))

# preallocate arrays
Qoverwash_total, Qoverwash_mat_total, Qinlet_total, Qinlet_mat_total, F, F_mat, dy_py, dt_py, nt_py, dy_mat, dt_mat = [
...
for ii in inputs_p1:
    for jj in inputs_p2:
        ...
        brie = output[ii, jj]
        dy[ii, jj] = brie._dy  # for debugging
        dt[ii, jj] = brie._dt
        nt[ii, jj] = np.size(brie._Qoverwash)

        # sum outputs for the same morphologic time (here, calculated as 1,000 years)
        if bnorm:
            Qoverwash_total[ii, jj] = np.mean(brie._Qoverwash / (brie._ny * brie._dy))
            Qinlet_total[ii, jj] = np.mean(brie._Qinlet / (brie._ny * brie._dy))
        else:
            Qoverwash_total[ii, jj] = np.mean(brie._Qoverwash)
            Qinlet_total[ii, jj] = np.mean(brie._Qinlet)

        F[ii, jj] = (Qinlet_total[ii, jj] /
                     (Qinlet_total[ii, jj] + Qoverwash_total[ii, jj]))
        # inlet_age = [brie._inlet_age]
        inlet_nr = [brie._inlet_nr]
        ...
        if bnorm:
            ...
            tmp_Qoverwash_mat = tmp_Qoverwash_mat / (brie._ny * brie._dy)
            tmp_Qinlet_mat = tmp_Qinlet_mat / (brie._ny * brie._dy)
        else:
            tmp_Qoverwash_mat = [mat['output'][ii][jj]['Qoverwash'].flat[0]]
            tmp_Qinlet_mat = [mat['output'][ii][jj]['Qinlet'].flat[0]]
...
#
#        t = np.arange(dt[ii,jj],(dt[ii,jj]*nt[ii,jj])+dt[ii,jj],brie._dtsave*dt[ii,jj])  # time array
#        ax = axs[0,0]
...
# in the current setup, [3,5 corresponds to dt = 0.05, dy = 100]
# [5,3] corresponds to dt = 0.1, dy = 400]
im = ax.pcolormesh(Qoverwash_total / Qoverwash_total[5, 3], edgecolors='white', linewidths=1,
                   antialiased=True)  # , vmin=0.8,vmax=1.2
fig.colorbar(im, ax=ax)
ax.set_title('python - Qoverwash')
...
ax = axs[1, 1]
im = ax.pcolormesh(Qoverwash_mat_total / Qoverwash_mat_total[5, 3], edgecolors='white', linewidths=1,
                   antialiased=True)
fig.colorbar(im, ax=ax)
ax.set_title('matlab - Qoverwash')
ax.set_xticks(np.arange(len(param2)) + 0.5)  # set ticks in the center of box
...
# Qinlet
ax = axs[0, 0]
im = ax.pcolormesh(Qinlet_total / Qinlet_total[5, 3], edgecolors='white', linewidths=1,
                   antialiased=True)
fig.colorbar(im, ax=ax)
ax.set_title('python - Qinlet')
ax.set_xticks(np.arange(len(param2)) + 0.5)  # set ticks in the center of box
...
ax = axs[1, 0]
im = ax.pcolormesh(Qinlet_mat_total / Qinlet_mat_total[5, 3], edgecolors='white', linewidths=1,
                   antialiased=True)
fig.colorbar(im, ax=ax)
ax.set_title('matlab - Qinlet')
ax.set_xticks(np.arange(len(param2)) + 0.5)  # set ticks in the center of box
...
# F
ax = axs[0, 2]
im = ax.pcolormesh(F / F[5, 3], edgecolors='white', linewidths=1,
                   antialiased=True)
fig.colorbar(im, ax=ax)
...
ax = axs[1, 2]
im = ax.pcolormesh(F_mat / F_mat[5, 3], edgecolors='white', linewidths=1,
                   antialiased=True)
fig.colorbar(im, ax=ax)
...
# Qoverwash
ax = axs[0, 1]
im = ax.pcolormesh(Qoverwash_total, edgecolors='white', linewidths=1,
                   antialiased=True)  # , vmin=0.8,vmax=1.2
fig.colorbar(im, ax=ax)
ax.set_title('python - Qoverwash')
...
ax = axs[1, 1]
im = ax.pcolormesh(Qoverwash_mat_total, edgecolors='white', linewidths=1,
                   antialiased=True)
fig.colorbar(im, ax=ax)
ax.set_title('matlab - Qoverwash')
ax.set_yticks(np.arange(len(param1)) + 0.5)  # set ticks in the center of box
...
# Qinlet
ax = axs[0, 0]
im = ax.pcolormesh(Qinlet_total, edgecolors='white', linewidths=1,
                   antialiased=True)
fig.colorbar(im, ax=ax)
ax.set_title('python - Qinlet')
ax.set_yticks(np.arange(len(param1)) + 0.5)  # set ticks in the center of box
...
ax = axs[1, 0]
im = ax.pcolormesh(Qinlet_mat_total, edgecolors='white', linewidths=1,
                   antialiased=True)
fig.colorbar(im, ax=ax)
ax.set_title('matlab - Qinlet')
ax.set_yticks(np.arange(len(param1)) + 0.5)  # set ticks in the center of box
...
# F
ax = axs[0, 2]
im = ax.pcolormesh(F, edgecolors='white', linewidths=1,
                   antialiased=True)
fig.colorbar(im, ax=ax)
ax.set_title('python - F')
ax.set_yticks(np.arange(len(param1)) + 0.5)  # set ticks in the center of box
